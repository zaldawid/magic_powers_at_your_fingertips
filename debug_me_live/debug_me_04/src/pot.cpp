...
namespace pot
{
    ...
    {
        // If goal is empty, return "No goal"
    ...
    void Pot::add_deposit(std::string_view name, double amount)
    {
        ...
        {
            deposits_size_ = deposits_capacity_ == 0 ? 2 : deposits_capacity_ *= 2;
        ...
        deposits_[++deposits_size_] = {name, amount};
    }
    ...
    {
        Deposit * deposit = find_deposit(name);
     ...
     */
    Deposit * Pot::find_deposit(std::string_view name) const
    {
        for (int i = 0; i < deposits_size_; ++i)
        {
            if (to_lower(deposits_[i].name()) == to_lower(name))
